...
;(function ( $, window, document, undefined ) {
	var pluginName = "waterripple",
        defaults = {
            delay		: 10,		//delay before re-drawing the next frame
    ...
    Plugin.prototype = {
        init: function() {
			...
			scope.ctx.drawImage(this.image[0], 0, 0,this.image[0].width,this.image[0].height,0,0,scope.width,scope.height);
			$(this.element).hide();
			...
			if(this.options.arbitrary > 0) {
				var rnd = Math.random;
				disturb.call(scope, 0.5 * scope.width, 0.4 * scope.height);
