...
$(function(){
	...
	COMMON_PLUGIN.COMMON.MOBILE_SCROLL_INIT($('#birthday'),opt1.maxDate,opt1.minDate,'');
	...
	function changeUserInfoDefaultHtml(obj){
		...
		var sexHtml = [];
		for(var k in sex){
			sexHtml.push('<option value="' + k + '" >' +  sex[k] + '</option>');
		}
		var identificationHtml = [];
		for(var k in identification){
			identificationHtml.push('<option value="' + k + '" >' +  identification[k] + '</option>');
		}
		var maritalStatusHtml = [];
		for(var k in maritalStatus){
			maritalStatusHtml.push('<option value="' + k + '" >' +  maritalStatus[k] + '</option>');
		}
		var interestingHtml = [];
		for(var k in interesting){
			interestingHtml.push('<div class="c-list-item">' + interesting[k] + '<input id="' + k + '" type="checkbox" name="cklike" value="' + k + '" /></div>');
		...
		$("#identification").html(identificationHtml.join(''));
		$("#maritalStatus").html(maritalStatusHtml.join(''));
		$("#interesting").html(interestingHtml.join(''));
	...
	//选择兴趣爱好点击效果
	function chooseListClick(){
		$('.choose-list').find('.c-list-item').on('click',function(){
			var $this = $(this);
	...
	function changeUserInfoHtml(obj){
		...
		if(interesting !== ""){
			...
			for(var i = 0, len = inter.length; i < len; i++){
				...
				$($("#" + inter[i]).parent()).addClass("selected");
			}
		}
		if(birthday !== ""){
	...
	function saveBtnUp(){
		//debugger
		var cksex = $("#sex").val() || "";
		var ckid = $("#identification").val() || "";
		var ckstatus = $("#maritalStatus").val() || "";
